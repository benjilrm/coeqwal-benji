...
addmargins(table(network_pwsids_w_gw_variables$gw_access, 
    ...
    margin = c(1, 2))
```
Version excluding consectuvie connections
```{r}
...
Export
```{r}
write.csv(network_pwsids_w_gw_variables%>%select(-violation_rate), "Data/Outputs/20250904_network_pwsids_w_gw_variables.csv", row.names = F)

write.csv(network_pwsids_w_gw_variables_excl_consecutive_connections%>%select(-violation_rate), "Data/Outputs/20250904_network_pwsids_w_gw_variables_excl_consecutive_connections.csv", row.names = F)
```
