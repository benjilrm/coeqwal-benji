...
## Network data
```{r}
network_raw = read_excel("Data/Source/Network data/Final data for joins and adding additional columns Dec 4 2025.xlsx", na = "NA")%>%
...
network_w_cleaned_swp_cvp = network_w_gw%>%
  ...
  mutate(
    ...
    colorado = ifelse(is.na(colorado), colorado_og, colorado))%>%
  select(-c(swp_og:colorado_og))
...
# Export
```{r}
write.csv(network_w_looped_projects, "Data/Outputs/20251208_network_joined.csv", row.names = F)
```
