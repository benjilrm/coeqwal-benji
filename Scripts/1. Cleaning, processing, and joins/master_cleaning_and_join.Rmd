...
## Network data
```{r}
network_raw = read.csv("Data/Source/Network data/Nov182025_finaldata_originalonly.csv")%>%
...
source_ids = network_w_lat_longs%>%
  ...
  ungroup()%>%
  mutate(source_id = ifelse(source_type == "PWS", source_name, group_id)) %>%
...
reviewed_all = rbind(reviewed_swp, reviewed_cvp)%>%
  ...
  rename(pwsid = 1, source_name = 3, swp = 7, cvp = 8, colorado = 9)%>%
  select(pwsid, source_name, swp, cvp, colorado)

network_w_cleaned_swp_cvp = network_w_gw%>%
  left_join(., reviewed_all, by = c("pwsid", "source_name"))%>%
  mutate(
    swp = ifelse(is.na(swp), swp_og, swp),
    cvp = ifelse(is.na(cvp), cvp_og, cvp),
    colorado = ifelse(is.na(colorado), colorado_incompleteandnotedited, colorado))%>%
  select(-c(swp_og:colorado_incompleteandnotedited))

rm(reviewed_swp, reviewed_cvp, reviewed_all)

network_w_cleaned_swp_cvp
```

Fill in existing NA project columns (which occur when source type is another PWS) by tracing back through the PWSs to the original sources
```{r}
#Create the new columns
network_w_cleaned_swp_cvp = network_w_cleaned_swp_cvp %>%
  mutate(
    swp_loop = replace_na(swp, 0),
    cvp_loop = replace_na(cvp, 0),
    colorado_loop = replace_na(colorado, 0))
...
network_w_looped_projects <- trace_sources(network_w_cleaned_swp_cvp)%>%
  mutate(
    swp_text = case_when(
      swp == 1 ~ "Direct",
      (swp == 0 | is.na(swp)) & swp_loop == 1 ~ "Indirect",
      (swp == 0 | is.na(swp)) & swp_loop != 1 ~ "No"),
    cvp_text = case_when(
      cvp == 1 ~ "Direct",
      (cvp == 0 | is.na(cvp)) & cvp_loop == 1 ~ "Indirect",
      (cvp == 0 | is.na(cvp)) & cvp_loop != 1 ~ "No"),
    colorado_text = case_when(
      colorado == 1 ~ "Direct",
      (colorado == 0 | is.na(colorado)) & colorado_loop == 1 ~ "Indirect",
      (colorado == 0 | is.na(colorado)) & colorado_loop != 1 ~ "No"))%>%
  select(
        ...
        connection_id, purchased:average_source_method, swp:colorado_text,
...
# Export
```{r}
write.csv(network_w_looped_projects, "Data/Outputs/20251125_network_joined.csv", row.names = F)
```
