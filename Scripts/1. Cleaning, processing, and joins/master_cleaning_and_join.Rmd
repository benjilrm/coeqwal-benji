...
## Network data
```{r}
network_raw = read_excel("Data/Source/Network data/UpdateddataforBenji_March92026.xlsx", na = "NA")
```
...
# Export
```{r}
network_w_looped_projects%>%
  group_by(pwsid)%>%
  reframe(n_sources = n(),
          sum = round(sum(average_source_usage)))%>%
  filter(sum != 100)

write.csv(network_w_looped_projects, "Data/Outputs/20260309_network_joined.csv", row.names = F)
```
