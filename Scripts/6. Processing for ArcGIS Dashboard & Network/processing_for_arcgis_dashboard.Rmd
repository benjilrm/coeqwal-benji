...
- Add new columns for # of systems served (for sources) and # of SW sources (for systems)
- Clean up errors in source type column
- Fix errors for 2+ different sources sharing same name when they are different sources
...
network_clean = network%>%
  ...
  mutate(
    ...
    source_lat = case_when(
      source_name == "Foresthill Public Utility Dist, Mill Creek" ~ 39.06713459841269,
      source_name == "Lake Havasu" ~ 34.29384,
      T ~ source_lat),
    source_lon = case_when(
      source_name == "Foresthill Public Utility Dist, Mill Creek" ~ -120.77255094125323,
...
# Export
```{r}
st_write(connections_sf, "Data/Outputs/Shapefiles for dashboard/20251112_connections.shp", delete_dsn = T)
st_write(connections_sf, "Data/Outputs/Shapefiles for dashboard/20251112_connections.gpkg", delete_dsn = T)
#st_write(connections_sf, "Data/Outputs/Shapefiles for dashboard/20251030_connections.geojson", delete_dsn = T)

st_write(systems_sf, "Data/Outputs/Shapefiles for dashboard/20251112_systems.shp", delete_dsn = T)
st_write(systems_sf, "Data/Outputs/Shapefiles for dashboard/20251112_systems.gpkg", delete_dsn = T)
#st_write(systems_sf, "Data/Outputs/Shapefiles for dashboard/20251030_systems.geojson", delete_dsn = T)

st_write(jittered_sources, "Data/Outputs/Shapefiles for dashboard/20251112_sources.shp", delete_dsn = T)
st_write(jittered_sources, "Data/Outputs/Shapefiles for dashboard/20251112_sources.gpkg", delete_dsn = T)
