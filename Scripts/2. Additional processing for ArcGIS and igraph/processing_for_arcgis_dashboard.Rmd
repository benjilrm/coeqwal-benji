...
```{r}
network = read.csv("Data/Outputs/20251017_network_joined.csv")
```
...
network_clean = network%>%
  ...
  select(-c(is_wholesale, source_summary, combined_source_notes, combined_calsim_notes, x2020_ccr_percent:x2023_percent, summer_2025_cleaning, calsim_demandunit:calsim_notes_sept2025, colorado_loop, gw_access:gw_compliant_w_dw_standards, swp:colorado, colorado_loop, connection_id, source_id, state_classification))%>%
...
network_w_lat_longs = network_clean%>%
  ...
  mutate(
    source_lat = ifelse(source_name == "Lake Havasu", 34.29384, source_lat),
    source_lon = ifelse(source_name == "Lake Havasu", -114.1686, source_lon))
```
...
# Export
```{r}
st_write(connections_sf, "Data/Outputs/Shapefiles for dashboard/20251120_connections.shp", delete_dsn = T)
st_write(connections_sf, "Data/Outputs/Shapefiles for dashboard/20251120_connections.gpkg", delete_dsn = T)
#st_write(connections_sf, "Data/Outputs/Shapefiles for dashboard/20251030_connections.geojson", delete_dsn = T)

st_write(systems_sf, "Data/Outputs/Shapefiles for dashboard/20251120_systems.shp", delete_dsn = T)
st_write(systems_sf, "Data/Outputs/Shapefiles for dashboard/20251120_systems.gpkg", delete_dsn = T)
#st_write(systems_sf, "Data/Outputs/Shapefiles for dashboard/20251030_systems.geojson", delete_dsn = T)

st_write(jittered_sources, "Data/Outputs/Shapefiles for dashboard/20251120_sources.shp", delete_dsn = T)
st_write(jittered_sources, "Data/Outputs/Shapefiles for dashboard/20251120_sources.gpkg", delete_dsn = T)
