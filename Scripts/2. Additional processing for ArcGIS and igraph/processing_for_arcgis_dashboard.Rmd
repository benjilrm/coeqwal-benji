...
```{r}
network = read.csv("Data/Outputs/20260309_network_joined.csv")
```
...
ADD CONNECTIONS ATTRIBUTES!
ex: if system OR source has gw or not
```{r}
network_clean = network_clean%>%
  ...
  mutate(any_swp_source = ifelse(any(swp == "Yes"), "Yes", "No"),
         ...
         any_gw_source = case_when(
           any(gw_access == "Groundwater access with 1 or more well meeting MCL requirements") ~ "Groundwater access with 1 or more well meeting MCL requirements",
           any(gw_access == "Groundwater access with no wells meeting MCL requirements") ~ "Groundwater access with no wells meeting MCL requirements",
           any(gw_access == "No data") ~ "No data",
           T ~ "No groundwater"))%>%
  ungroup()%>%
  group_by(pwsid) %>%
  mutate(
...
network_clean%>%
  filter(system_name == "SAN FRANCISCO REGIONAL WATER SYSTEM" | source_name == "CA3810001")
```
...
# Export
```{r}
st_write(connections_sf, "Data/Outputs/Shapefiles for dashboard/20260309_connections.shp", delete_dsn = T)
#st_write(connections_sf, "Data/Outputs/Shapefiles for dashboard/20251125_connections.gpkg", delete_dsn = T)

st_write(systems_sf, "Data/Outputs/Shapefiles for dashboard/20260309_systems.shp", delete_dsn = T)
#st_write(systems_sf, "Data/Outputs/Shapefiles for dashboard/20251125_systems.gpkg", delete_dsn = T)

st_write(jittered_sources, "Data/Outputs/Shapefiles for dashboard/20260309_sources.shp", delete_dsn = T)
