...
network_clean = network_clean%>%
  ...
  mutate(any_swp_source = ifelse(any(swp == "Yes"), "Yes", "No"),
         ...
         any_gw_source = case_when(
           any(gw_access == "No drinkable groundwater") ~ "No drinkable groundwater"))%>%
  ungroup()%>%
  group_by(pwsid) %>%
  mutate(
...
systems_sf = network_w_lat_longs%>%
  select(system_name, pwsid, system_lat, system_lon, provider_type, county, pop_served, num_sw_sources, gw_access, system_size, any_swp_system, any_cvp_system, any_purchased_system)%>%
  unique()%>%
...
# Export
```{r}
st_write(connections_sf, "Data/Outputs/Shapefiles for dashboard/202603125_connections.shp", delete_dsn = T)

st_write(systems_sf, "Data/Outputs/Shapefiles for dashboard/20260325_systems.shp", delete_dsn = T)
st_write(systems_sf%>%filter(!is.na(project)), "Data/Outputs/Shapefiles for dashboard/20260325_project_systems.shp", delete_dsn = T)

st_write(jittered_sources, "Data/Outputs/Shapefiles for dashboard/20260325_sources.shp", delete_dsn = T)
```
