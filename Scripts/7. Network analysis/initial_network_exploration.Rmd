...
```{r}
dwr_vulnerability = read.csv("Data/Source/DWR Small System Vulnerability/i07_Water_Shortage_Vulnerability_Small_Water_Systems.csv")%>%
  select(SABL_PWSID, SC5e_Drought_Impact)%>%
  rename(pwsid = 1, dwr_drought = 2)%>%
  mutate(dwr_drought = as.factor(ifelse(grepl("Listed as having", dwr_drought), 1, 0)))
```
...
rm(path_lengths, path_length_matrix)
```
...
system_metrics_comb = left_join(system_centrality, system_path_lengths, by = "pwsid")%>%
  ...
  left_join(., all_nodes%>%
              ...
              rename(pwsid = id), 
            by = "pwsid")%>%
  left_join(., dwr_vulnerability, by = "pwsid")
  
#rm(diversity_df, system_centrality, system_path_lengths, dwr_vulnerability)

table(system_metrics_comb%>%filter(pop_served <= 10000)%>%pull(dwr_drought), useNA = "always")
```
...
```{r}
ggpairs(system_metrics_comb[c(2:6, 11)])+
...
summary_stats = system_metrics_comb%>%
  filter(!is.na(dwr_drought))%>%
  group_by(dwr_drought)%>%
  reframe(in_degree = mean(in_degree),
          mean_path_length = mean(mean_path_length),
          hhi = mean(hhi))%>%
  mutate(dwr_drought = ifelse(dwr_drought == 0, "no_drought_vulnerability_mean", "drought_vulnerability_mean"))
...
mann_whitney_results <- system_metrics_comb %>%
  summarise(
    in_degree_test = list(wilcox.test(in_degree ~ dwr_drought)),
    hhi_test = list(wilcox.test(hhi ~ dwr_drought)),
    mean_path_length_test = list(wilcox.test(mean_path_length ~ dwr_drought)))
...
Tests indicate that small systems not designated as drought-vulnerable tend to have lower in-degree (ie: less sources) and greater path length (ie: more removed from their original sources) than drought-vulnerable systems. HHI is not significant. 
