...
```{r, eval = F}
system_adjacency = bipartite_projection(network)$proj2
...
system_metrics_comb = left_join(system_centrality, system_path_lengths, by = "pwsid")%>%
  ...
  mutate(drought_vulnerable = ifelse(pop_served > 3300, NA, drought_vulnerable))
  
#rm(diversity_df, system_centrality, system_path_lengths, dwr_vulnerability)

table(system_metrics_comb%>%filter(pop_served <= 3300)%>%pull(drought_vulnerable), useNA = "always")
```
