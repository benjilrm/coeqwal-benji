#Generate AI data for all available urban water management plans
...
#List of UWMPs for analysis
file_list <- read_csv("Data_raw/UWMP_list.csv")
...
for (i in 1:length(file_list$PWSID)){
  pdf_path <- paste0(pdf_path1, file_list[i,5], pdf_path2)
...
At the end of your summary, list any emergency water sources or interconnections the system has as backup for water supply if mentioned using the header **Backup sources**:")
...
sink("Documents/OpenAIdata_UWMP_all.txt")
print(list)
sink()
