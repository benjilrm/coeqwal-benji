...
```{r}
load("Data/Outputs/20260601_network_igraph.Rdata")

network_df = read.csv("Data/Outputs/20260601_network_joined.csv")
``` 
...
Drought vulnerability from USDA FOIA
```{r}
usda = read_excel("Data/Source/Small System Drought Vulnerability/Responsive Records 2026-RD-01323-F.xlsx")%>%
  rename(pwsid = 2)%>%
  select(pwsid)%>%
  filter(!pwsid %in% comb_vulnerability$pwsid & grepl("CA", pwsid))
```
...
comb_vulnerability = full_join(dwr_vulnerability, swrcb_vulnerability, by = "pwsid")%>%
  ...
  full_join(., usda, by = "pwsid")%>%
  mutate(across(where(is.numeric), ~replace_na(.x, 0)),
         any_drought_vulnerable = 1)%>%
  unique()

rm(dwr_vulnerability, swrcb_vulnerability, pacific_vulnerability, usda)
```
