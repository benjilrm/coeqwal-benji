...
```{r}
load("data for upload/20251215_network_igraph.Rdata")
...
system_metrics_comb%>%filter(pop_served <= 10000)%>%
  ...
  labs(title = "System Source Diversity Metric", x = "Herfindahl–Hirschman Index", y = "# of Systems") +
  theme_bw()
```
...
table(system_metrics_comb$fully_self_supplied)
...
wilcox.test(n_reachable_sources ~ self_supplied, data = system_metrics_comb)
aggregate(n_reachable_sources ~ self_supplied, data = system_metrics_comb, FUN = mean)


wilcox.test(hhi ~ fully_self_supplied, data = system_metrics_comb)
aggregate(hhi ~ fully_self_supplied, data = system_metrics_comb, FUN = mean)

wilcox.test(in_degree ~ fully_self_supplied, data = system_metrics_comb)
aggregate(in_degree ~ fully_self_supplied, data = system_metrics_comb, FUN = mean)

wilcox.test(mean_path_length ~ fully_self_supplied, data = system_metrics_comb)
aggregate(mean_path_length ~ fully_self_supplied, data = system_metrics_comb, FUN = mean)

wilcox.test(n_reachable_sources ~ fully_self_supplied, data = system_metrics_comb)
aggregate(n_reachable_sources ~ fully_self_supplied, data = system_metrics_comb, FUN = mean)
```
...
```{r}
table(system_metrics_comb$drought_vulnerable)
table(system_metrics_comb$pacific_drought_vulnerable)
```
...
wilcox.test(n_reachable_sources ~ drought_vulnerable, data = system_metrics_comb)
aggregate(n_reachable_sources ~ drought_vulnerable, data = system_metrics_comb, FUN = mean)


wilcox.test(hhi ~ pacific_drought_vulnerable, data = system_metrics_comb)
aggregate(hhi ~ pacific_drought_vulnerable, data = system_metrics_comb, FUN = mean)

wilcox.test(in_degree ~ pacific_drought_vulnerable, data = system_metrics_comb)
aggregate(in_degree ~ pacific_drought_vulnerable, data = system_metrics_comb, FUN = mean)

wilcox.test(mean_path_length ~ pacific_drought_vulnerable, data = system_metrics_comb)
aggregate(mean_path_length ~ pacific_drought_vulnerable, data = system_metrics_comb, FUN = mean)
```
...
system_metrics_comb = system_metrics_comb%>%
  select(-c(system_name:pop_served, purchased, pacific_drought_vulnerable, total_degree, median_path_length:min_path_length))%>%
...
features <- system_metrics_comb %>%
  select(-c(pwsid, system_size, self_supplied, fully_self_supplied, hhi, num_sw_sources))
...
system_metrics_comb$role_cluster <- kmeans_result$cluster
