...
self_supplied = network_df%>%
  ...
  reframe(percent_not_purchased = sum(average_source_usage))%>%
  mutate(self_supplied = ifelse(percent_not_purchased >= 50, 1, 0),
         fully_self_supplied = ifelse(percent_not_purchased == 100, 1, 0))%>%
  select(-percent_not_purchased)

system_metrics_comb = system_metrics_comb%>%
  left_join(., self_supplied, by = "pwsid")%>%
  replace_na(list(self_supplied = 0, fully_self_supplied = 0))
