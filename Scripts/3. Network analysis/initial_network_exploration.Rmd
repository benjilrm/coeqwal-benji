...
```{r}
dwr_vulnerability2 = read_excel("Data/Source/Small System Drought Vulnerability/Compiled DWR PRA impacted systems.xlsx")%>%
...
```{r}
comb_vulnerability = full_join(dwr_vulnerability1, dwr_vulnerability2, by = "pwsid")%>%
  full_join(., pacific_vulnerability, by = "pwsid")%>%
  mutate(across(where(is.numeric), ~replace_na(.x, 0)),
         any_drought_vulnerable = 1)

rm(dwr_vulnerability1, dwr_vulnerability2, pacific_vulnerability)
```
...
system_metrics_comb = left_join(system_centrality, system_path_lengths, by = "pwsid")%>%
  ...
  mutate(across(c(sb552_drought_vulnerable, dwr_drought_vulnerable, any_drought_vulnerable, pacific_drought_vulnerable), ~ replace_na(., 0)))
...
length(which(source_metrics_comb$out_degree <= 1))/nrow(source_metrics_comb)
length(which(system_metrics_comb$total_degree <= 1))/nrow(system_metrics_comb)
...
features <- system_metrics_comb %>%
  select(-c(pwsid, system_size, pop_served, system_size_agg, self_supplied, fully_self_supplied, num_sw_sources, system_name:state_classification, purchased, sb552_drought_vulnerable, pacific_drought_vulnerable, dwr_drought_vulnerable, total_degree, median_path_length:min_path_length, gw_access:gw_compliant_w_dw_standards))
...
clusters = small_system_metrics_comb %>%
  ...
  select(-c(pwsid, total_degree, median_path_length:min_path_length, system_name:state_classification, system_size:num_sw_sources, sb552_drought_vulnerable:pacific_drought_vulnerable, purchased, gw_compliant_w_dw_standards))%>%
...
library(kableExtra)
key_summary = clusters %>%
  select(mclust_cluster, n, in_degree:hhi, mean_source_out_degree:mean_source_n_reachable_systems, percent_purchased, self_supplied:fully_self_supplied, pop_served, pop_na, gw_access, drought_vulnerable)%>%
  mutate(across(where(is.numeric), round, 2)) %>%
  kbl(col.names = c("Cluster", "Size",  "In-Degree", "Out-Degree", "Betweenness", 
                    "Mean Path Length", "# Reachable Sources", "# Reachable Systems", "HHI", "Mean Source Out-Degree", "Mean Source # of Reachable Systems", "% Volume Purchased", "Self-Supplied", "Fully Self-Supplied", "Pop. Served", "Systems with No Pop. Served", "GW Access", "Drought Vulnerable")) %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
  
key_summary

writeLines(key_summary, "Manuscript outputs/cluster_table_mclust.html")
```
