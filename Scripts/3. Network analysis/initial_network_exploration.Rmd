...
features <- system_metrics_comb %>%
  select(-c(pwsid, system_size, pop_served, system_size_agg, self_supplied, fully_self_supplied, num_sw_sources, system_name:state_classification, purchased, drought_vulnerable, pacific_drought_vulnerable, total_degree, median_path_length:min_path_length, gw_access:gw_compliant_w_dw_standards))
...
mclust_result <- Mclust(features)

# Get optimal number of clusters and variables
summary(mclust_result)

# Add to data
clusters = system_metrics_comb %>%
  mutate(mclust_cluster = mclust_result$classification,
         pop_na = ifelse(is.na(pop_served), 1, 0))%>%
  select(-c(pwsid, total_degree, median_path_length:min_path_length, system_name:state_classification, system_size:num_sw_sources, pacific_drought_vulnerable:purchased, gw_compliant_w_dw_standards))%>%
