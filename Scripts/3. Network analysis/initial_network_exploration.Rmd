...
```{r}
nrow(system_metrics_comb%>%filter(system_size_agg == "Small" & pwsid %in% network_df$source_id))
...
key_summary = clusters %>%
  #select(mclust_cluster, n, in_degree:hhi, mean_source_out_degree:mean_source_n_reachable_systems, percent_purchased, self_supplied:fully_self_supplied, pop_served, pop_na, gw_access, any_drought_vulnerable)%>%
  mutate(across(where(is.numeric), round, 2)) 
  #kbl(col.names = c("Cluster", "Size",  "In-Degree", "Out-Degree", "Betweenness", "Mean Path Length", "# Reachable Sources", "# Reachable Systems", "HHI", "Mean Source Out-Degree", "Mean Source # of Reachable Systems", "% Volume Purchased", "Self-Supplied", "Fully Self-Supplied", "Pop. Served", "Systems with No Pop. Served", "GW Access", "Drought Vulnerable")) %>%
