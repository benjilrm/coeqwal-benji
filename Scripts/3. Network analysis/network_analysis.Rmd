...
network_df%>%
  ...
  filter(n_distinct(cvp_loop) > 1)%>%
  arrange(source_name)%>%
  select(system_name:pwsid, source_name:source_type, cvp_text, cvp_loop, cvp_direct)
``` 
...
```{r}
nrow(system_metrics_comb%>%filter(self_supplied != 1))
nrow(system_metrics_comb%>%filter(fully_self_supplied != 1))

nrow(small_system_metrics_comb%>%filter(self_supplied != 1))
nrow(small_system_metrics_comb%>%filter(fully_self_supplied != 1))
```
...
summary(lm(betweenness ~ pop_served, data = system_metrics_comb))
```
...
cluster_percentiles <- clusters %>%
  select(mclust_cluster, all_of(vars_to_plot)) %>%
  rowwise() %>%
  mutate(across(all_of(vars_to_plot), 
                ~ ecdf(small_system_metrics_comb[[cur_column()]])(.),
                .names = "pctl_{.col}")) %>%
  ungroup() %>%
  select(mclust_cluster, starts_with("pctl_"))
