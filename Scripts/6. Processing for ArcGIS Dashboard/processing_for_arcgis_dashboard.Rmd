...
network_clean = network%>%
  ...
  mutate(
    ...
    source_type = case_when(
      ...
      T ~ source_type),
    source_id = as.numeric(gsub("source_", "", source_id)))%>%
...
sources_sf = network_clean %>%
  select(source_name, source_type, source_lat, source_lon, source_id) %>%
  group_by(source_name)%>%
  mutate(num_systems_served = n())%>%
  ungroup()%>%
...
table(connections_sf$gw_access, useNA = "always")

connections_sf%>%data.frame()%>%
  filter(is.na(gw_access))
```
