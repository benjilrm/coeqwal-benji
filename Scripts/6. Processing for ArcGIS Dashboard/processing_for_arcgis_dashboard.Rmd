...
```{r}
network = read_excel("data/Source/Network data/Final data 10.29.25 latlon comp.xlsx", na = "NA")
```

```{r}
master_system_lat_longs = read_excel("Data/Source/Network data/20251029_unified system latlons.xlsx")%>%
  rename_with(tolower)%>%
  rename(override_lat = latitude, override_lon = longitude)%>%
  select(-name)

master_system_lat_longs%>%
  arrange(override_lat)%>%
  group_by(override_lat, override_lon)%>%
  filter(n() > 1)
```
